<template>
  <div style="height: 2000px">
      ...
      <div id="container" class="resume-container">
        ...
        <div id="sidebar" class="resume-catalogue">
          <div class="resume-catalogue-group">
            <h2>简历目录</h2>
            <ul>
              <a v-for="item in sidebarData" :key="item.label" style="color: #5f687c; display: block"
                :href="'#' + item.label">{{ item.label }}</a>
            </ul>
          </div>
        </div>
        ...
        <div id="resume-content" class="resume-content">
          <div class="resume-content-box">
            ...
            <div class="resume-box">
              <!-- 个人信息 -->
              <div id="个人信息" class="resume-item resume-userInfo resume-baseInfo" @mouseenter="edit = true"
                @mouseleave="edit = false">
                <div class="item-primary baseInfo-container">
                  <div class="user-info">
                    <div class="info-flex" @click="editingData(0)" v-show="sidebarData[0].editing != true">
                      <p>xxx同学</p>
                      <div style="margin: 8px 0">
                        <span>
                          <i class="fz-resume" style="
                  ...
                  <div v-show="sidebarData[0].editing">
                    <div style="font-size:16px;font-weight: 500;margin-bottom:10px">编辑个人信息</div>
                    <el-form :inline="true" ref="form" :model="sizeForm" label-width="80px" size="mini">
                      ...
                      <el-form-item label="生日">
                        <el-date-picker v-model="sizeForm.birthday" type="date" placeholder="选择日期">
                        </el-date-picker>
                      </el-form-item>
                      ...
                      <el-form-item size="mini">
                        <el-button type="primary" @click="oneSubmit(0)">立即创建</el-button>
              ...
              <div class="resume-item resume-summary resume-userDesc">
                ...
                <div id="个人优势" class="item-primary">
                  ...
                  <div id="data" class="data" @click="editingData(1)" @mouseenter="editInformation(1)"
                    @mouseleave="leaveInformation(1)" v-show="sidebarData[1].editing != true">
                    {{ personalInformation }}
                    <i v-show="sidebarData[1].choose" class="el-icon-edit edit">编辑</i>
                  </div>
                  <!-- 个人优势编辑 -->
                  <div v-show="sidebarData[1].editing">
                    <el-input type="textarea" :rows="4" placeholder="请输入内容" v-model="personalInformation" />
                    <span style="display: block; margin: 10px 0 0 85%">
                      <el-button size="mini" @click="oneCancel(1)">取消</el-button>
                      <el-button size="mini" type="primary" @click="oneSubmit(1)">确定</el-button>
                    </span>
                  </div>
                </div>
                <!-- 希望职位 -->
                <div id="希望职位" class="item-primary">
                  ...
                  <div id="data" class="data" @click="editingData(2)" @mouseenter="editInformation(2)"
                    @mouseleave="leaveInformation(2)" v-show="sidebarData[2].editing != true">
                    <span style="line-height: 40px">
                      <i class="el-icon-s-flag">{{ form.name }}</i>
                      <el-divider direction="vertical" />
                      <i style="margin: 0 10px" class="el-icon-money">{{ form.start
                        }}<span v-show="form.start != '' && form.start != '面谈'">k--</span>{{ form.end }}<span
                          v-show="form.end != ''">k</span></i>
                      <el-divider direction="vertical" />
                      <i style="margin: 0 10px" class="el-icon-data-line">{{
                        form.industry.toString()
                      }}</i>
                      <el-divider direction="vertical" />
                      <i style="margin: 0 0 0 10px" class="el-icon-location-information">{{ form.workCity }}</i>
                    </span>
                    <i v-show="sidebarData[2].choose" class="el-icon-edit edit">编辑</i>
                  </div>
                  <!-- 希望职位编辑 -->
                  <div v-show="sidebarData[2].editing">
                    <el-form :inline="true" size="mini" ref="form" :model="form" label-width="80px">
                      <el-form-item label="期望职位">
                        <el-input style="width: 150px" v-model="form.name" @focus="selectPositionDia"></el-input>
                      </el-form-item>
                      <el-form-item label="工作城市">
                        <el-cascader v-model="form.region" ref="getCity" placeholder="" :props="props"
                          :options="regions" filterable clearable @change="selectCity" :show-all-levels="false">
                        </el-cascader>
                      </el-form-item>
                      <el-form-item label="期望行业">
                        <el-input ref="formIndustry" v-model="form.industry" placeholder="请选择活动区域"
                          @focus="selectIndustryCategories('expect')">
                        </el-input>
                      </el-form-item>
                      <el-form-item label="期望薪资">
                        <el-select @change="selectStartMoney" style="width: 150px" v-model="form.start">
                          <el-option label="面谈" value="面谈"></el-option>
                          <el-option v-for="(item, index) in startingSalary" :key="index" :label="item + 'k'"
                            :value="item">
                          </el-option>
                        </el-select>
                        <span v-show="form.start != '面谈'">
                          ...
                          <el-select style="width: 150px" v-model="form.end">
                            <el-option v-for="(item, index) in endSalary" :key="index" :label="item + 'k'"
                              :value="item">
                            </el-option>
                          </el-select>
                        </span>
                      ...
                      <el-form-item>
                        <span>
                          <el-button style="margin-right: 8px" @click="oneCancel(2)">取消</el-button>
                        </span>
                        <el-button type="primary" @click="oneSubmit(2)">立即创建</el-button>
                      </el-form-item>
                    </el-form>
                  </div>
                </div>
                ...
                <div id="实习经历" class="item-primary">
                  <h3 class="title">
                    实习经历
                    <span v-show="!sidebarData[3].editing"><i class="el-icon-circle-plus" @click="addCard">添加</i></span>
                  </h3>
                  <div id="data" class="data" v-for="(val, index) in cardData" :key="index"
                    @click="editingData(3, index)" @mouseenter="editInformation(3)" @mouseleave="leaveInformation(3)"
                    v-show="sidebarData[3].editing != true">
                    <el-card class="box-card">
                      <div slot="header" class="clearfix">
                        <span>卡片名称</span>
                        <span class="time">时间</span>
                        <i v-show="sidebarData[3].choose" style="float: right" class="el-icon-edit edit">编辑</i>
                        <i @click.stop="deleteCard(index)" v-show="sidebarData[3].choose" style="float: right"
                          class="el-icon-delete-solid edit">删除</i>
                      </div>
                      <h4>
                        <span>铲平不</span>
                        <span class="two">家教</span>
                      </h4>
                      <div class="info-text">
                  ...
                  <div v-show="sidebarData[3].editing">
                    <el-form :inline="true" size="mini" ref="form" :model="internshipExperience" label-width="80px">
                      <el-form-item label="公司名称">
                        <el-input style="width: 150px" v-model="internshipExperience.name" />
                      </el-form-item>
                      <el-form-item label="所属行业">
                        <el-input ref="internshipExperienceIndustry" @focus="selectIndustryCategories('internship')"
                          style="width: 150px" v-model="internshipExperience.industry" />
                      </el-form-item>
                      <el-form-item label-width="130px" label="所属部门（选填）">
                        <el-input v-model="internshipExperience.department" placeholder="" />
                      </el-form-item>
                      <el-form-item label="职位名称">
                        <el-input @focus="selectJobTitle" v-model="internshipExperience.position" placeholder="" />
                      </el-form-item>

                      <el-form-item label="在职时间">
                        <el-date-picker v-model="workingTime" type="monthrange" range-separator="至"
                          @change="selectWorkingTime" start-placeholder="开始日期" end-placeholder="结束日期"
                          format="yyyy 年 MM 月 " value-format="yyyy-MM">
                        </el-date-picker>
                      </el-form-item>
                      <el-form-item label="工作内容">
                        <el-input :autosize="{ minRows: 4, maxRows: 8 }" style="width: 100vh" type="textarea"
                          placeholder="请输入内容" v-model="internshipExperience.textarea" maxlength="1600"
                          show-word-limit />
                      </el-form-item>
                      <br />
                      <el-form-item style="margin: 0 0 0 70%">
                        <span>
                          <el-button style="margin-right: 8px" @click="oneCancel(3)">取消</el-button>
                          <el-button type="primary" @click="oneSubmit(3)">立即创建</el-button>
                        </span>
                      </el-form-item>
                    </el-form>
                  </div>
    ...
    <!-- 希望职位--》行业类别 -->
    <el-dialog title="请选择行业类别" :visible.sync="dialogVisibleIndustryCategories" width="70%">
      <div style="border-top: 1px solid #f5f6f9">
        <span>
          <el-tag size="medium" closable style="margin: 5px; color: #37e2da; border-radius: 20px"
            v-for="(item, index) in form.industry" :key="item" effect="plain" @close="clearTag(index)">
            {{ item }}
          </el-tag>
        </span>
        <span style="
      ...
      <div class="industry-panel">
        <div v-for="(item, index) in industryCategories" :key="index" class="industry-item">
          <div class="left">
            {{ item.name }}
          </div>
          <div class="right">
            <div class="item" v-for="(val, inde) in item.subLevelModelList" :key="inde"
    ...
    <!-- 实习经历--》所属行业 -->
    <el-dialog title="请选择行业类别" :visible.sync="dialogVisibleIndustryCategoriesShiXi" width="70%">
      <div class="industry-panel">
        <div v-for="(item, index) in industryCategories" :key="index" class="industry-item">
          <div class="left">
            {{ item.name }}
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisibleIndustryCategoriesShiXi = false">取 消</el-button>
        <el-button type="primary" @click="dialogVisibleIndustryCategoriesShiXi = false">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 希望职位--》期望职位  -->
    <el-dialog title="请选择职类" :visible.sync="dialogVisiblePosition" width="60%">
      <div class="diaPosition">
        <div class="left">
          <ul>
            <li v-for="(item, index) in position" :key="index" :class="{ active: positionActive == index }"
        ...
        <div class="right">
          <ul>
            <li v-for="(item, index) in positionRightData" @click="selectPosition(item.name)" :key="index">
              {{ item.name }}
            </li>
          </ul>
        </div>
      ...
      <span style="margin-top: 10px" slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogVisiblePosition = false">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 实习经历--》职位名称  -->
    <el-dialog title="请选择职位名称" :visible.sync="dialogVisibleExpectPosition" width="60%">
      <div class="diaPosition">
        <div class="left">
          <ul>
            <li v-for="(item, index) in expectposition" :key="index" :class="{ active: expectpositionActive == index }"
        ...
        <div class="right">
          <ul>
            <li v-for="(item, index) in expectpositionRightData" @click="selectExpectPosition(item.name)" :key="index">
              {{ item.name }}
            </li>
          </ul>
        </div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisibleExpectPosition = false">取 消</el-button>
        <el-button type="primary" @click="dialogVisibleExpectPosition = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
